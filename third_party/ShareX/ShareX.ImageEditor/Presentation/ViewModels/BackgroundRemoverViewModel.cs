...
public sealed partial class BackgroundRemoverViewModel : ViewModelBase, IDisposable
{
    ...
    private async Task BrowseImageAsync()
    {
        ...
        string? filePath = await SelectImageFileRequested(EditorLocalizer.SelectImage);
    ...
    private async Task RemoveBackgroundAsync()
    {
        ...
        try
        {
            ...
            stopwatch.Stop();
            ShowNotification(EditorLocalizer.BackgroundRemovedIn(stopwatch.ElapsedMilliseconds), EditorIcons.ToolSmartEraser);
    ...
    private async Task SaveImageAsync(Func<SKBitmap, string?, Task<string?>>? saveRequested, string notificationIcon)
    {
        ...
        try
        {
            ...
            {
                ShowNotification(EditorLocalizer.ImageSavedNotification(savedPath), notificationIcon);
            }
        }
        catch (Exception ex)
        {
