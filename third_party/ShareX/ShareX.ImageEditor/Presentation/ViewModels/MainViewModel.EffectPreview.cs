...
namespace ShareX.ImageEditor.Presentation.ViewModels
{
    public partial class MainViewModel : ViewModelBase
    {
        ...
        {
            ApplyOneShotEffect(img => new InvertImageEffect().Apply(img), EditorLocalizer.InvertedColors);
        }

        [RelayCommand]
        private void BlackAndWhite()
        {
            ApplyOneShotEffect(img => new BlackAndWhiteImageEffect().Apply(img), EditorLocalizer.AppliedBlackAndWhiteFilter);
        }

        [RelayCommand]
        private void Sepia()
        {
            ApplyOneShotEffect(img => new SepiaImageEffect().Apply(img), EditorLocalizer.AppliedSepiaFilter);
        }

        [RelayCommand]
        private void Polaroid()
        {
            ApplyOneShotEffect(img => new PolaroidImageEffect().Apply(img), EditorLocalizer.AppliedPolaroidFilter);
        }

        [RelayCommand]
        private void EdgeDetect()
        {
            ApplyOneShotEffect(img => new EdgeDetectImageEffect().Apply(img), EditorLocalizer.AppliedEdgeDetectFilter);
        }

        [RelayCommand]
        private void Emboss()
        {
            ApplyOneShotEffect(img => new EmbossImageEffect().Apply(img), EditorLocalizer.AppliedEmbossFilter);
        }

        [RelayCommand]
        private void MeanRemoval()
        {
            ApplyOneShotEffect(img => new MeanRemovalImageEffect().Apply(img), EditorLocalizer.AppliedMeanRemovalFilter);
        }

        [RelayCommand]
        private void Smooth()
        {
            ApplyOneShotEffect(img => new SmoothImageEffect().Apply(img), EditorLocalizer.AppliedSmoothFilter);
        }
