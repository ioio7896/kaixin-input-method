...
public sealed class ToolbarCustomizationDialogViewModel : ViewModelBase
{
    ...
    private void RefreshValidation()
    {
        ...
        foreach (ToolbarCustomizationItemViewModel item in Items.Where(item => item.IsHotkeyEditable))
        {
            ...
            {
                ValidationMessage = EditorLocalizer.InvalidHotkey(item.Name);
                return;
            }
            ...
            {
                ValidationMessage = EditorLocalizer.DuplicateHotkey(item.Name, existingItem.Name, normalizedHotkey);
                return;
            }
