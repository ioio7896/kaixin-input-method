...
namespace ShareX.ImageEditor.Presentation.ViewModels
{
    public partial class MainViewModel : ViewModelBase
    {
        ...
        public void ShowOpenImageNotification(string? filePath)
        {
            ...
            ShowNotification(BuildFilePathNotification(EditorLocalizer.ImageOpened, filePath), EditorIcons.FileOpen);
        }

        public void ShowNewImageNotification(int width = 0, int height = 0)
        {
            ShowNotification(BuildImageSizeNotification(EditorLocalizer.NewImageCreated, width, height), EditorIcons.FileNew);
        }

        public void ShowImageCroppedNotification(int width = 0, int height = 0)
        {
            ShowNotification(BuildImageSizeNotification(EditorLocalizer.ImageCropped, width, height), EditorIcons.ToolCrop);
        }

        public void ShowImageCutOutNotification(int width = 0, int height = 0)
        {
            ShowNotification(BuildImageSizeNotification(EditorLocalizer.ImageCutOut, width, height), EditorIcons.ToolCutOut);
        }

        public void ShowImageInsertedNotification()
        {
            ShowNotification(EditorLocalizer.ImageInserted, EditorIcons.ToolImage);
        }

        public void ShowImageAutoCroppedNotification()
        {
            ShowNotification(EditorLocalizer.ImageAutoCropped, EditorIcons.ToolCrop);
        }

        public void ShowImageResizedNotification()
        {
            ShowNotification(EditorLocalizer.ImageResized, EditorIcons.ToolImage);
        }

        public void ShowCanvasResizedNotification()
        {
            ShowNotification(EditorLocalizer.CanvasResized, EditorIcons.PanelBackground);
        }

        public void ShowImageRotatedClockwiseNotification()
        {
            ShowNotification(EditorLocalizer.ImageRotatedClockwise, EditorIcons.ActionRotateRight);
        }

        public void ShowImageRotatedCounterClockwiseNotification()
        {
            ShowNotification(EditorLocalizer.ImageRotatedCounterClockwise, EditorIcons.ActionRotateLeft);
        }

        public void ShowImageRotated180Notification()
        {
            ShowNotification(EditorLocalizer.ImageRotated180, EditorIcons.ActionRotateRight);
        }
        ...
        public void ShowImageRotatedCustomAngleNotification(float angle)
        {
            ...
            ShowNotification(EditorLocalizer.ImageRotatedBy(formattedAngle), EditorIcons.ActionRotateRight);
        }

        public void ShowImageFlippedHorizontallyNotification()
        {
            ShowNotification(EditorLocalizer.ImageFlippedHorizontally, EditorIcons.PanelEffects);
        }

        public void ShowImageFlippedVerticallyNotification()
        {
            ShowNotification(EditorLocalizer.ImageFlippedVertically, EditorIcons.PanelEffects);
        }
        ...
        public void ShowEffectAppliedNotification(string? statusMessage)
        {
            ...
            {
                ShowNotification(EditorLocalizer.ImageEffectApplied, EditorIcons.PanelEffects);
                return;
            }
        ...
        private void ShowSaveNotification(string? savedPath, string icon)
        {
            ...
            ShowTaskActionNotification(BuildFilePathNotification(EditorLocalizer.ImageSavedToFile, savedPath), icon);
        }

        private static string BuildFilePathNotification(string headline, string filePath)
        {
            return $"{headline}\n{string.Format(CultureInfo.CurrentCulture, EditorLocalizer.FilePathLabel, filePath)}";
        }

        private static string BuildImageSizeNotification(string headline, int width, int height)
        {
            return width > 0 && height > 0
                ? $"{headline}\n{EditorLocalizer.SizeLabel(width, height)}"
                : headline;
        }
