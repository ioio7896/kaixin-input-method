...
namespace ShareX
{
    public class TaskSettings
    {
        ...
        // Transient handoff used only by the bundled Kaixin capture command.
        // It is deliberately excluded from persisted ShareX task settings.
        [JsonIgnore]
        public string KaixinOutputPath = "";

        // Per-request completion handoff for the bundled Kaixin capture flow.
        // ShareX writes captured/cancelled/failed after the capture task has
        // actually finished so the tray never has to infer cancellation from
        // a long clipboard timeout.
        [JsonIgnore]
        public string KaixinResultPath = "";
