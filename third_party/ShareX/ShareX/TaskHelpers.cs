...
namespace ShareX
{
    public static class TaskHelpers
    {
        ...
        private static Bitmap AnnotateImageModern(Bitmap bmp, string filePath, TaskSettings taskSettings, bool taskMode = false)
        {
            ...
            {
                // Kaixin capture commands reuse the persisted default editor
                // options. Save as soon as the editor closes so preferences are
                // not lost if the background ShareX process is later stopped by
                // the tray, an upgrade, or Windows shutdown.
