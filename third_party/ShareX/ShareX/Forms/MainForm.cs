...
namespace ShareX
{
    public partial class MainForm : HotkeyForm
    {
        ...
        private async void MainForm_HandleCreated(object sender, EventArgs e)
        {
            ...
            if (!Program.KaixinIntegration && Program.Settings.ActionsToolbarRunAtStartup)
            {
        ...
        public async Task UpdateControls()
        {
            ...
            niTray.Visible = Program.Settings.ShowTray && !Program.KaixinIntegration;
        ...
        private async Task InitHotkeys()
        {
            ...
            if (Program.KaixinIntegration)
            {
                DebugHelper.WriteLine("HotkeyManager disabled for Kaixin integration mode.");
                return;
            }
...
#pragma warning restore WFO5001
        ...
        private void AfterApplicationSettingsJobs()
        {
            ...
            // Kaixin consumes the accepted editor result through the clipboard.
            // Publish the lossless PNG format as well as the compatibility bitmap
            // so the handoff never depends on GDI's BGRA/alpha conversion.
            HelpersOptions.UseAlternativeClipboardCopyImage =
                Program.KaixinIntegration || Program.Settings.UseAlternativeClipboardCopyImage;
        ...
        {
            if (value && !IsHandleCreated && (Program.KaixinIntegration ||
                ((Program.SilentRun || Program.Settings.SilentRun) && Program.Settings.ShowTray)))
            {
                CreateHandle();
                value = false;
            }
