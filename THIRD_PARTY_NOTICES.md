...
原生截图路径使用 Windows Graphics Capture；当 WGC 不可用或捕获失败时，可回退到 DXGI Desktop Duplication。智能框选、窗口吸附和控件吸附均由开心输入法本地实现，最终图像仅在本机处理，再按用户设置复制、保存或交给 OCR。
...
| `lexicon/zh/life_common_4char.txt`、`lexicon/zh/life_common_phrases_5to8.txt`、`lexicon/zh-ext/daily_communication.txt` | 现代生活、聊天和办公短语 | 词频排序来自 wordfreq 3.1.1（Apache-2.0）；项目脚本补充并整理现代固定搭配，源片段及生成方法见 `data_sources/lexicon_fragments/zh-ext/` 与 `scripts/build_common_phrase_lexicons.py` |
...
| `lexicon/zh-ext/technology.txt` | 现代 AI、软件云、互联网产品和编程框架实体 | 词频排序来自 wordfreq 3.1.1（Apache-2.0）；项目脚本补充现代技术实体和中英混合输入码，源片段及生成方法见 `data_sources/lexicon_fragments/zh-ext/` 与 `scripts/build_modern_it_lexicons.py` |
| `lexicon/zh-ext/people_names.txt` | 中国姓氏及常见人物姓名 | 姓氏源自 `chinese-surnames` 1.0.0（MIT）；上游来源、版本和许可证原文见 `data_sources/README.md` 与 `data_sources/chinese_surnames/LICENSE` |
| `data_sources/pinyin/`、`pinyin-ime/data/pinyin_supported_chars.txt` | 拼音生成与支持字符表 | rust-pinyin 0.10.0，MIT License；字符表由构建工具重新生成 |
| `data_sources/kaixin/*.tsv`、`lexicon/core/kaixin_polyphone.txt` | 基础候选、多音词读音修正及其生成结果 | 开心输入法项目作者使用生成式 AI 辅助添加并自行维护；见 `docs/licenses/AI_DATA_PROVENANCE.md` |
| `lexicon/zh-ext/animals.txt`、`data_sources/kaixin/*.tsv` | 项目自维护中文词库与读音数据 | 项目作者使用生成式 AI 辅助生成、整理、校正并人工筛选；见 `docs/licenses/AI_DATA_PROVENANCE.md` |
| `lexicon/zh-ext/geography_admin.txt`、`lexicon/zh-ext/hangzhou_local.txt` | 行政区划及杭州本地词库 | 项目作者使用生成式 AI 辅助添加并自行维护；可追溯源片段位于 `data_sources/lexicon_fragments/zh-ext/` |
...
- 生成 Python 依赖许可证报告，覆盖 `.python-packages` 和 `.python-runtime` 中随包分发的依赖。
- 保留 `data_sources/pinyin/LICENSE` 和 `data_sources/README.md`。
