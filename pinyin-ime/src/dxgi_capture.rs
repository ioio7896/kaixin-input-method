...
mod tests {
    ...
    fn monitor(id: u32, x: i32, y: i32, width: u32, height: u32) -> MonitorInfo {
        MonitorInfo {
            id,
    ...
    fn dxgi_output(
        ...
        DxgiOutputIdentity {
            ...
            output_index,
            monitor_id,
            device_name: format!("output-{adapter_index}-{output_index}"),
            bounds: CaptureRect::new(x, y, width, height),
        }
    }

    #[test]
    fn virtual_bounds_supports_negative_coordinates() {
        let bounds = virtual_bounds(&[
            monitor(1, 0, 0, 1920, 1080),
            monitor(2, -1280, -100, 1280, 1024),
        ])
        .unwrap();
        assert_eq!(bounds, CaptureRect::new(-1280, -100, 3200, 1180));
    }

    #[test]
    fn intersection_crops_window_to_monitor() {
        let window = CaptureRect::new(-100, 30, 400, 200);
        let monitor = CaptureRect::new(0, 0, 1920, 1080);
        assert_eq!(
            intersection(window, monitor),
            Some(CaptureRect::new(0, 30, 300, 200))
        );
    }

    #[test]
    fn invalid_regions_are_rejected_before_allocating() {
        assert!(validate_rect(CaptureRect::new(0, 0, 0, 10)).is_err());
        assert!(validate_rect(CaptureRect::new(0, 0, u32::MAX, u32::MAX)).is_err());
    }

    #[test]
    fn output_mapping_uses_hmonitor_and_coordinates_for_equal_sized_multi_gpu_outputs() {
        let monitors = vec![
            monitor(0x101, 0, 0, 1920, 1080),
            monitor(0x202, 1920, 0, 1920, 1080),
        ];
        let outputs = vec![
            dxgi_output(1, 0, 0x202, 1920, 0, 1920, 1080),
            dxgi_output(0, 0, 0x101, 0, 0, 1920, 1080),
        ];
        assert_eq!(
            map_dxgi_outputs_to_monitors(&outputs, &monitors).unwrap(),
            vec![
                OutputMonitorMapping {
                    output_index: 0,
                    monitor_index: 1,
                },
                OutputMonitorMapping {
    ...
    #[test]
    fn output_mapping_rejects_coordinate_mismatch_even_when_id_and_size_match() {
        let monitors = vec![monitor(0x101, 0, 0, 1920, 1080)];
        let outputs = vec![dxgi_output(0, 0, 0x101, -1920, 0, 1920, 1080)];
        let err = map_dxgi_outputs_to_monitors(&outputs, &monitors).unwrap_err();
        assert_eq!(err.kind, CaptureErrorKind::Backend);
        assert!(err.message.contains("坐标"));
    }

    #[test]
    fn output_mapping_rejects_two_outputs_claiming_one_monitor() {
        let monitors = vec![monitor(0x101, 0, 0, 1920, 1080)];
        let outputs = vec![
            dxgi_output(0, 0, 0x101, 0, 0, 1920, 1080),
            dxgi_output(1, 0, 0x101, 0, 0, 1920, 1080),
        ];
        let err = map_dxgi_outputs_to_monitors(&outputs, &monitors).unwrap_err();
        assert_eq!(err.kind, CaptureErrorKind::Backend);
        assert!(err.message.contains("同一显示器"));
    }

    #[cfg(windows)]
    #[test]
    #[ignore = "requires an interactive Windows desktop"]
    fn dxgi_virtual_desktop_smoke() {
        let frame = capture_virtual_desktop().unwrap();
    ...
    #[test]
    #[ignore = "requires an interactive Windows desktop"]
    fn dxgi_foreground_window_smoke() {
        use windows::Win32::UI::WindowsAndMessaging::GetForegroundWindow;

        let hwnd = unsafe { GetForegroundWindow() };
        assert!(!hwnd.is_invalid());
        let frame = capture_window(hwnd.0 as isize).unwrap();
        assert!(frame.width() > 0);
        assert!(frame.height() > 0);
    }
}
