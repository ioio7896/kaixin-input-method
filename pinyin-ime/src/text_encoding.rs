...
mod tests {
    ...
    fn decodes_utf8_without_bom() {
        let text = decode_text_bytes("开心输入法\n".as_bytes()).unwrap();
        assert_eq!(text, "开心输入法\n");
    }

    #[test]
    fn decodes_utf8_with_bom() {
        let bytes = [0xEF, 0xBB, 0xBF, b'a', b'b', b'c'];
        let text = decode_text_bytes(&bytes).unwrap();
        assert_eq!(text, "abc");
    }
    ...
    fn decodes_utf16le_with_bom() {
        let bytes = [
            0xFF, 0xFE, 0x00, 0x5F, 0xC3, 0x5F, 0x93, 0x8F, 0x65, 0x51, 0xD5, 0x6C,
        ];
        let text = decode_text_bytes(&bytes).unwrap();
        assert_eq!(text, "开心输入法");
    }
    ...
    fn decodes_utf16be_with_bom() {
        let bytes = [
            0xFE, 0xFF, 0x5F, 0x00, 0x5F, 0xC3, 0x8F, 0x93, 0x51, 0x65, 0x6C, 0xD5,
        ];
        let text = decode_text_bytes(&bytes).unwrap();
        assert_eq!(text, "开心输入法");
    }
    ...
    #[test]
    fn decodes_utf16le_without_bom_when_pattern_matches() {
        let bytes = [b'I', 0, b'\t', 0, b'b', 0, b'j', 0, b'\n', 0];
        let text = decode_text_bytes(&bytes).unwrap();
        assert_eq!(text, "I\tbj\n");
    }
}
