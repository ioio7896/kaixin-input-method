...
mod tests {
    ...
    #[test]
    fn user_dict_encryption_defaults_enabled() {
        assert_eq!(encryption_enabled(), cfg!(windows));
    }

    #[test]
    fn user_dict_sqlite_round_trips_contents() {
        let path = std::env::temp_dir().join("kx_user_dict_io_round_trip.sqlite");
        let _ = fs::remove_file(&path);

        assert_eq!(read_user_dict_sqlite_text(&path).unwrap(), None);
        ...
        .unwrap();
        assert_eq!(
            read_user_dict_sqlite_text(&path).unwrap().unwrap(),
            "I\tbj\t北京\t1\t1\t0\nI\tsh\t上海\t1\t2\t0\nU\tshuru\t数入\t-3\t3\nX\t打开\tshuru\t输入法\t4\t4\nY\t打开\tshuru\t数入\t-3\t5\nD\tI\tbj\t\t北京\t65536\t65536\t1\nE\tbj\t北京\t2\t1\t1"
        );
    ...
    #[test]
    fn user_dict_plain_sqlite_export_round_trips_contents() {
        let path = std::env::temp_dir().join("kx_user_dict_io_export_round_trip.sqlite");
        let _ = fs::remove_file(&path);

        write_plain_user_dict_sqlite(&path, "I\thello\thello\t7\t9\t1\n").unwrap();
        let raw = fs::read(&path).unwrap();
        assert!(!raw.starts_with(ENCRYPTED_MAGIC));
        assert_eq!(
            read_user_dict_sqlite_export_text(&path).unwrap(),
            "I\thello\thello\t7\t9\t1"
        );
    ...
    #[cfg(windows)]
    #[test]
    fn user_dict_sqlite_contents_are_dpapi_wrapped_on_disk() {
        let path = std::env::temp_dir().join("kx_user_dict_io_encrypted.sqlite");
        let _ = fs::remove_file(&path);

        write_user_dict_sqlite_snapshot(&path, "I\tbj\t北京\t1\t1\t0\n".as_bytes()).unwrap();
        let raw = fs::read(&path).unwrap();
        assert!(raw.starts_with(ENCRYPTED_MAGIC));
        assert!(!raw.windows("北京".len()).any(|w| w == "北京".as_bytes()));
        assert_eq!(
            read_user_dict_sqlite_text(&path).unwrap().unwrap(),
            "I\tbj\t北京\t1\t1\t0"
        );
    ...
    #[cfg(windows)]
    #[test]
    fn live_user_dictionary_rejects_plaintext_sqlite() {
        let path = std::env::temp_dir().join("kx_user_dict_plaintext_rejected.sqlite");
        write_plain_user_dict_sqlite(&path, "I\thello\thello\t1\t1\t0\n").unwrap();
        assert!(read_user_dict_sqlite_text(&path).is_err());
        let _ = fs::remove_file(&path);
    }
}
