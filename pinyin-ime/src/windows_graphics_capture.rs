...
mod tests {
    ...
    #[test]
    fn rectangle_intersection_handles_negative_monitor_coordinates() {
        let selection = CaptureRect::new(-100, 20, 300, 100);
        let monitor = CaptureRect::new(-1920, 0, 1920, 1080);
        assert_eq!(
            intersection(selection, monitor),
            Some(CaptureRect::new(-100, 20, 100, 100))
        );
    }

    #[test]
    fn virtual_bounds_stitches_monitors_on_both_sides() {
        let bounds = virtual_bounds([
            CaptureRect::new(-1280, 0, 1280, 1024),
            CaptureRect::new(0, -200, 1920, 1080),
        ])
        .unwrap();
        assert_eq!(bounds, CaptureRect::new(-1280, -200, 3200, 1224));
    }

    #[test]
    fn rejects_empty_and_unreasonably_large_regions() {
        assert!(CaptureRect::new(0, 0, 0, 10).validate().is_err());
        assert!(CaptureRect::new(0, 0, u32::MAX, u32::MAX)
            .validate()
            .is_err());
    }

    #[test]
    fn fallback_classification_is_explicit() {
        let error = CaptureError::new(CaptureErrorKind::Timeout, "timeout");
        assert!(error.should_fallback());
        let error = CaptureError::new(CaptureErrorKind::InvalidTarget, "bad hwnd");
        assert!(!error.should_fallback());
    }

    #[test]
    fn captured_pixels_preserve_rgb_when_becoming_opaque() {
        let mut image = RgbaImage::from_raw(
            3,
            1,
            vec![
                64, 32, 16, 128, // RGB is already the final desktop colour
                12, 34, 56, 0, // drivers can leave alpha empty on desktop frames
                7, 8, 9, 255, // already opaque
            ],
        )
        .unwrap();

        normalize_opaque_capture(&mut image);

        assert_eq!(
            image.as_raw(),
            &[64, 32, 16, 255, 12, 34, 56, 255, 7, 8, 9, 255]
        );
    }
    ...
    fn wgc_primary_monitor_smoke() {
        assert!(is_supported());
        let frame = capture(CaptureTarget::PrimaryMonitor).unwrap();
