...
impl Matcher {
    ...
    pub fn with_params(params: &MatcherParams) -> Matcher {
        ...
        init_score_tables(
            &mut res.direction_score_table,
            &mut res.length_score_table,
            &mut res.pos_score_table,
        );
    ...
    pub fn lookup(&mut self, strokes: &Vec<Stroke>, collector: &mut MatchCollector) {
        ...
        CHAR_DATA.with(|char_data| {
            ...
            let maximum_strokes = usize::min(
                stroke_count + stroke_range,
                self.params.MAX_CHARACTER_STROKE_COUNT,
            );
            ...
            let sub_strokes_range =
                self.get_sub_strokes_range(sub_stroke_count, self.params.DEFAULT_LOOSENESS);
            let min_sub_strokes = usize::max(sub_stroke_count - sub_strokes_range, 1);
            let max_sub_strokes = usize::min(
                sub_stroke_count + sub_strokes_range,
            ...
            for cix in 0..char_data.len() {
                ...
                if (cmp_stroke_count as usize) < minimum_strokes
                    || cmp_stroke_count as usize > maximum_strokes
                {
                    continue;
                }
                if cmp_sub_strokes.len() < min_sub_strokes
                    || cmp_sub_strokes.len() > max_sub_strokes
                {
                    continue;
                }
                // Match against character in repo
                let char_match = self.match_one(
                    stroke_count,
                    &input_sub_strokes,
                    sub_strokes_range,
                    repo_char,
    ...
    fn match_one(
        ...
        if input_stroke_count == repo_char.stroke_count as usize
            && input_stroke_count < self.params.CORRECT_NUM_STROKES_CAP
        {
            ...
            let bonus = self.params.CORRECT_NUM_STROKES_BONUS
                * (i32::max(
                    self.params.CORRECT_NUM_STROKES_CAP as i32 - input_stroke_count as i32,
                    0,
                ) as f32)
        ...
        for x in 0..input_sub_strokes.len() {
            ...
            for y in 0..repo_char.sub_strokes.len() {
                ...
                if range <= sub_strokes_range {
                    ...
                    let match_score = self.compute_sub_stroke_score(
                        input_direction,
                        input_length,
                        cmp_dir,
                        cmp_length,
    ...
    fn compute_sub_stroke_score(
        ...
        let direction_score =
            self.get_direction_score(input_direction, repo_direction, input_length);
    ...
    fn init_score_matrix(&mut self) {
        ...
        for i in 0..self.score_matrix.len() {
            let penalty = -self.params.AVG_SUBSTROKE_LENGTH
                * self.params.SKIP_PENALTY_MULTIPLIER
                * (i as f32);
    ...
    fn get_strokes_range(&self, stroke_count: usize, looseness: f32) -> usize {
        ...
        if looseness == 1f32 {
            return self.params.MAX_CHARACTER_STROKE_COUNT;
        }
        ...
        let curve = CubicCurve2D::new(
            ...
            ctrl2_y,
            1.0,
            self.params.MAX_CHARACTER_STROKE_COUNT as f32,
        );
    ...
    fn get_sub_strokes_range(&self, sub_stroke_count: usize, looseness: f32) -> usize {
        ...
        if looseness == 1.0 {
            return self.params.MAX_CHARACTER_SUB_STROKE_COUNT;
        }
        // We use a CubicCurve that grows slowly at first and then rapidly near the end to the maximum.
        ...
        let curve = CubicCurve2D::new(
            ...
            ctrl2_y,
            1.0,
            self.params.MAX_CHARACTER_SUB_STROKE_COUNT as f32,
        );
        let t = curve.get_first_solution_for_x(looseness);
        // We get the t value on the parametrized curve where the x value matches the looseness.
        // Then we compute the y value for that t. This gives the range.
...
fn init_score_tables(
    direction_score_table: &mut Vec<f32>,
    length_score_table: &mut Vec<f32>,
    pos_score_table: &mut Vec<f32>,
) {
