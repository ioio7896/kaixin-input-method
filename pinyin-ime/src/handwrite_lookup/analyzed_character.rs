...
mod tests {
    ...
    fn assert_same(sample_anc: &SampleAnChar, anc: &AnalyzedCharacter) {
        assert!(
            sample_anc.analyzedStrokes.len() == anc.analyzed_strokes.len(),
            "Expected same number of analyzed strokes."
        );
        for as_ix in 0..sample_anc.analyzedStrokes.len() {
            ...
            assert!(
                sample_ans.points.len() == ans.points.len(),
                "Analyzed stroke expected to have same number of points."
            );
            for pt_ix in 0..sample_ans.points.len() {
                assert!(
                    sample_ans.points[pt_ix][0] == ans.points[pt_ix].x,
                    "Analyzed stroke expected to have the exact same points."
                );
                assert!(
                    sample_ans.points[pt_ix][1] == ans.points[pt_ix].y,
                    "Analyzed stroke expected to have the exact same points."
                );
            }
            // Analyze stroke must have same pivot indexes
            assert!(
                sample_ans.pivotIndexes.len() == ans.pivot_indexes.len(),
                "Analyzed stroke expected to have same number of pivot indexes."
            );
            for pivot_ix in 0..sample_ans.pivotIndexes.len() {
                assert!(
                    sample_ans.pivotIndexes[pivot_ix] == ans.pivot_indexes[pivot_ix],
                    "Analyzed stroke expected to have the exact same pivot indexes."
                );
            }
            // Analyze stroke must have same substrokes
            assert!(
                sample_ans.subStrokes.len() == ans.sub_strokes.len(),
                "Analyzed stroke expected to have same number of substrokes."
            );
            for ss_ix in 0..sample_ans.subStrokes.len() {
                assert!(
                    sample_ans.subStrokes[ss_ix].direction as f32
                        == ans.sub_strokes[ss_ix].direction,
                    "Analyzed stroke must have the exact same substrokes."
                );
                assert!(
                    sample_ans.subStrokes[ss_ix].length as f32 == ans.sub_strokes[ss_ix].length,
                    "Analyzed stroke must have the exact same substrokes."
                );
                assert!(
                    sample_ans.subStrokes[ss_ix].centerX as f32 == ans.sub_strokes[ss_ix].center_x,
                    "Analyzed stroke must have the exact same substrokes."
                );
                assert!(
                    sample_ans.subStrokes[ss_ix].centerY as f32 == ans.sub_strokes[ss_ix].center_y,
                    "Analyzed stroke must have the exact same substrokes."
                );
            }
        }
