...
struct SingleCharacterStage;

impl CandidateStage for SingleCharacterStage {
    fn apply(&self, _context: &CandidatePipelineContext, ranked: &mut Vec<RankedCandidate>) {
        ranked.retain(|item| phrase_char_count(&item.phrase) == 1);
    }
}

struct TraditionalOutputStage;

impl CandidateStage for TraditionalOutputStage {
    fn apply(&self, _context: &CandidatePipelineContext, ranked: &mut Vec<RankedCandidate>) {
        let mut seen = HashSet::new();
...
#[path = "lookup/intent.rs"]
mod intent;
pub(super) use intent::*;
#[path = "lookup/pagination.rs"]
mod pagination;
